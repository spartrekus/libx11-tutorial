	      ...
	      short pixels[1000][1000])
{
  ...
  //XDrawArc(display, expose_event->window, gc, 50-(30/2), 100-(30/2), 30, 30, 0, 360*64);
  ...
  /* perform an events loop */
  {
    ...
    while (done == 0) {
      ...
      if (an_event.type == KeyPress)
      {
            ...
            if ( an_event.xkey.keycode == 9 ) done = 1;
      }
      ...
      switch (an_event.type) 
      {
        ...
        case Expose:
          /* redraw our window. */
	  handle_expose(display, gc, rev_gc, (XExposeEvent*)&an_event.xexpose, width, height, pixels);
          break;
        ...
        case ButtonPress:
          ...
          handle_button_down(display, gc, rev_gc, (XButtonEvent*)&an_event.xbutton, width, height, pixels);
          break;
  
        case MotionNotify:
          /* invert the pixel under the mouse pointer. */
          handle_drag(display, gc, rev_gc, (XButtonEvent*)&an_event.xbutton, width, height, pixels);

          XButtonEvent* drag_event = &an_event.xbutton;
          int xu = drag_event->x;
          int yu = drag_event->y;
          XFillRectangle(display, win , gc , xu , yu  , 5 , 5 );

          //XFillRectangle(display, window, DefaultGC(display, s), x1 , y1 , 5 , 5 );
          XFillRectangle(display, win , gc , 10  , 10  , 5 , 5 );
	  break;
  
        case KeyPress:
          /* exit the application by exiting out of the events loop. */
          //  done = 1;
          break;
